...
- [ ] **1.8** *(Nou, 2026-08-04)* Confirmar, inspeccionant el codi del plugin `wpml-translation-management` un cop instal·lat, els noms reals de les funcions/classes internes de creació de jobs i export/import d'XLIFF. Veure `BIBLIOGRAFIA.md` §11.
- [ ] **1.9** *(Nou, 2026-08-04)* Implementar `POST /create-job`, `GET /export-xliff/{job_id}`, `POST /import-xliff` a `gnss-bridge`, embolcallant les funcions confirmades a 1.8.
  - *Verificació:* crear un job de prova per a `translation_bot` (traductor local, `WPML → Translation Management → Translators`), exportar-ne l'XLIFF via l'endpoint, i confirmar que el fitxer és equivalent al que genera el botó "Export" de l'admin de WPML.
...
- [x] **8.1** `app/wordpress/wpml.py`: `link_translation()` (FASE 2.4) disponible com a funció del mòdul. *(Fet 2026-08-04 — no hi ha una classe `WPMLAdapter` pròpiament, són funcions pures al mòdul `wpml.py`, seguint el mateix estil que `content.py`. L'orquestració real "QA PASS → crida `link_translation()`" és feina de `app/cli/translate.py`, encara pendent (8.2) i bloquejada per WPML per a la prova end-to-end.)*
